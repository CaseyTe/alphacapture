...
function resolveEventStreamConfig(input) {
  ...
  const eventStreamPayloadHandler2 = input.eventStreamPayloadHandlerProvider({
    ...input,
    messageSigner
  });
  return {
    ...input,
    eventSigner,
    eventStreamPayloadHandler: eventStreamPayloadHandler2
  };
}
...
var websocketEndpointMiddleware = (config, options) => (next) => (args) => {
  var _a, _b;
  const { request } = args;
  if (HttpRequest.isInstance(request) && ((_b = (_a = config.requestHandler.metadata) == null ? void 0 : _a.handlerProtocol) == null ? void 0 : _b.toLowerCase().includes("websocket"))) {
...
var injectResponseValuesMiddleware = (config) => (next) => async (args) => {
  if (args.input.SessionId === void 0 && isWebSocket(config)) {
    args.input.SessionId = v4_default();
  }
  const requestParams = {
    ...args.input
  };
...
var websocketPortMiddleware = (options) => (next) => (args) => {
  var _a, _b;
  const { request } = args;
  if (HttpRequest.isInstance(request) && ((_b = (_a = options.requestHandler.metadata) == null ? void 0 : _a.handlerProtocol) == null ? void 0 : _b.includes("websocket"))) {
...
var q = [l];
var r = [{ [v]: "Region" }];
var _data = { version: "1.0", parameters: { Region: h, UseDualStack: i, UseFIPS: i, Endpoint: h }, rules: [{ conditions: [{ [t]: b, [u]: [j] }], rules: [{ conditions: p, error: "Invalid Configuration: FIPS and custom endpoint are not supported", type: d }, { conditions: q, error: "Invalid Configuration: Dualstack and custom endpoint are not supported", type: d }, { endpoint: { url: j, properties: m, headers: m }, type: e }], type: f2 }, { conditions: [{ [t]: b, [u]: r }], rules: [{ conditions: [{ [t]: "aws.partition", [u]: r, assign: g }], rules: [{ conditions: [k, l], rules: [{ conditions: [{ [t]: c, [u]: [a, n] }, o], rules: [{ endpoint: { url: "https://transcribestreaming-fips.{Region}.{PartitionResult#dualStackDnsSuffix}", properties: m, headers: m }, type: e }], type: f2 }, { error: "FIPS and DualStack are enabled, but this partition does not support one or both", type: d }], type: f2 }, { conditions: p, rules: [{ conditions: [{ [t]: c, [u]: [n, a] }], rules: [{ endpoint: { url: "https://transcribestreaming-fips.{Region}.{PartitionResult#dnsSuffix}", properties: m, headers: m }, type: e }], type: f2 }, { error: "FIPS is enabled but this partition does not support FIPS", type: d }], type: f2 }, { conditions: q, rules: [{ conditions: [o], rules: [{ endpoint: { url: "https://transcribestreaming.{Region}.{PartitionResult#dualStackDnsSuffix}", properties: m, headers: m }, type: e }], type: f2 }, { error: "DualStack is enabled but this partition does not support DualStack", type: d }], type: f2 }, { endpoint: { url: "https://transcribestreaming.{Region}.{PartitionResult#dnsSuffix}", properties: m, headers: m }, type: e }], type: f2 }], type: f2 }, { error: "Invalid Configuration: Missing Region", type: d }] };
var ruleSet = _data;
...
var getRuntimeConfig = (config) => {
  return {
    ...
    httpAuthSchemeProvider: (config == null ? void 0 : config.httpAuthSchemeProvider) ?? defaultTranscribeStreamingHttpAuthSchemeProvider,
    ...
    serviceId: (config == null ? void 0 : config.serviceId) ?? "Transcribe Streaming",
...
var getRuntimeConfig2 = (config) => {
  ...
  return {
    ...
    eventStreamPayloadHandlerProvider: (config == null ? void 0 : config.eventStreamPayloadHandlerProvider) ?? (() => eventStreamPayloadHandler),
...
var throwDefaultError2 = withBaseException(TranscribeStreamingServiceException);
